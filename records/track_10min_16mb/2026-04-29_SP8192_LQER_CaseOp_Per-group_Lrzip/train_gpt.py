...
class RMSNorm(nn.Module):
    def __init__(self, dim=0, eps=None):
        super().__init__()
        self.eps = eps
        self.weight = nn.Parameter(torch.ones(dim)) if dim > 0 else None
...
def _similarity_sort_l1(matrix):
    # Greedy nearest-neighbour row permutation under L1. Returns a uint16 row
    # order so lrzip sees consecutive rows that are byte-similar. O(n^2) but
    # only runs once at serialize time on matrices that fit in memory.
    import numpy as _np
    n = matrix.shape[0]
...
def _lrzip_compress(data, tmpdir, label):
    # lrzip needs real files on disk (no stdin pipe mode in the version we ship).
    # tmpdir is owned by serialize() / deserialize() and cleaned up there.
...
def _pack_streams(streams):
    # Layout: b"PGRP" | u32 n_streams | u32 len_0 ... u32 len_{n-1} | payloads.
    import struct
    n = len(streams)
...
def _compress(data, compressor):
    # The pergroup path is handled directly inside serialize() and never
    # reaches _compress; guard against accidental misrouting.
    if compressor == "pergroup":
        raise RuntimeError(
            "pergroup compressor must be handled by _serialize_pergroup, "
            "not _compress; check serialize() branch."
        )
...
def _decompress(data, compressor):
    # pergroup is detected via magic header in deserialize(), not here.
    if compressor == "lzma":
...
def serialize(h, base_model, code):
    ...
    if h.compressor == "pergroup":
        # Ported from train_gpt_new.py serialize(). lrzip is invoked via a
        # tempdir of scratch files; we create + clean the dir ourselves.
        import tempfile
...
def deserialize(h, device):
    ...
    # Auto-detect the pergroup format regardless of h.compressor. This keeps
    # eval-time compat with artifacts produced by either path.
